...
{
    /*---------- Controlador buscar código de producto ----------*/
    ...
    /*---------- Controlador agregar producto a venta ----------*//*---------- Controlador agregar producto a venta ----------*/
public function agregarProductoCarritoControlador()
{
    ...
    if (empty($_SESSION['datos_producto_venta'][$codigo])) {
        ...
        $mensaje_stock_bajo = "";
        if ($stock_total < 5) {
            $mensaje_stock_bajo = "<p class='has-text-danger' style='font-size: 30px; font-weight: bold;'>¡¡ Recuerde solicitar más de este producto !!</p>";
        }

        $_SESSION['alerta_producto_agregado'] = "Se agregó <strong>" . $campos['producto_nombre'] . "</strong> a la venta. Cantidad en inventario: <strong>$stock_total</strong>$mensaje_stock_bajo";
    } else {
        ...
        $mensaje_stock_bajo = "";
        if ($stock_total < 5) {
            $mensaje_stock_bajo = "<p class='has-text-danger' style='font-size: 16px; font-weight: bold;'>Recuerde solicitar más de este producto</p>";
        }

        $_SESSION['alerta_producto_agregado'] = "Se agregó +1 <strong>" . $campos['producto_nombre'] . "</strong> a la venta. Total en carrito: <strong>$detalle_cantidad</strong>. Cantidad en inventario: <strong>$stock_total</strong>$mensaje_stock_bajo";
    }
    ...
    public function removerProductoCarritoControlador()
    {
        ...
        if (!isset($_SESSION['datos_producto_venta'][$codigo])) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
                "texto" => "No hemos encontrado el producto que desea remover en el carrito",
                "icono" => "error"
            ];
            return json_encode($alerta);
            exit();
        ...
        $_SESSION['datos_producto_venta'][$codigo]['producto_stock_total'] = $_SESSION['datos_producto_venta'][$codigo]['producto_stock_total_old'];
    ...
    public function actualizarProductoCarritoControlador()
    {
        ...
        if (!empty($_SESSION['datos_producto_venta'][$codigo])) {
            ...
            if ($cantidad > $_SESSION['datos_producto_venta'][$codigo]["venta_detalle_cantidad"]) {
                $diferencia_productos = "agregó +" . ($cantidad - $_SESSION['datos_producto_venta'][$codigo]["venta_detalle_cantidad"]);
            } else {
                $diferencia_productos = "quitó -" . ($_SESSION['datos_producto_venta'][$codigo]["venta_detalle_cantidad"] - $cantidad);
            }
            ...
            $_SESSION['alerta_producto_agregado'] = "Se $diferencia_productos <strong>" . $campos['producto_nombre'] . "</strong> a la venta. Total en carrito <strong>$detalle_cantidad</strong>. Cantidad en inventario: <strong>$stock_total</strong>";
    ...
    public function registrarVentaControlador()
    {
        ...
        if (!preg_match("/^[0-9.]{1,25}$/", $venta_pagado)) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
    ...
    public function listarVentaControlador($pagina, $registros, $url, $busqueda)
    {
        ...
        $campos_tablas = "venta.venta_id, venta.venta_codigo, venta.venta_fecha, venta.venta_hora, venta.venta_total, usuario.usuario_nombre, usuario.usuario_apellido, cliente.cliente_nombre, cliente.cliente_apellido";

        if (isset($busqueda) && $busqueda != "") {
            $consulta_datos = "SELECT $campos_tablas FROM venta 
                INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id 
                INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id 
                WHERE (venta.venta_codigo LIKE '%$busqueda%') 
                ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";

            $consulta_total = "SELECT COUNT(venta_id) FROM venta WHERE (venta.venta_codigo LIKE '%$busqueda%')";
        } else {
            $consulta_datos = "SELECT $campos_tablas FROM venta 
                INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id 
                INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id 
                ...
                $tabla .= '
                    <tr class="has-text-centered">
                        ...
                        <td>
                            ...
                            <a href="' . APP_URL . 'saleDetail/' . $rows['venta_codigo'] . '/" class="button is-link is-rounded is-small" title="Información de venta Nro. ' . $rows['venta_id'] . '">
