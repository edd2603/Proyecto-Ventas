...
class userController extends mainModel {
    ...
    public function registrarUsuarioControlador() {
        ...
        $tipoUser = $this->limpiarCadena($_POST['rol_usuario']);
        ...
        $weakPasswords = ['123456', 'password', '123456789', '12345678', '12345', '1234567', '1234567890', 'qwerty', 'abc123'];

        if (in_array($clave1, $weakPasswords)) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
                "texto" => "La contraseña es débil o común. Por favor, usa una combinación de letras mayúsculas, minúsculas, números y caracteres especiales.",
                "icono" => "error"
            ];
            return json_encode($alerta);
            exit();
        ...
        if (strlen($clave1) < 8) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
                "texto" => "La contraseña es demasiado corta. Usa al menos 8 caracteres.",
                "icono" => "error"
            ];
            return json_encode($alerta);
            exit();
        ...
        $hasUpperCase = preg_match('/[A-Z]/', $clave1);
        $hasLowerCase = preg_match('/[a-z]/', $clave1);
        $hasNumbers = preg_match('/[0-9]/', $clave1);
        $hasSpecialChars = preg_match('/[$@.-]/', $clave1);

        if (!$hasUpperCase || !$hasLowerCase || !$hasNumbers || !$hasSpecialChars) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
                "texto" => "La contraseña es moderada. Usa una combinación de letras mayúsculas, minúsculas, números y caracteres especiales.",
                "icono" => "error"
            ];
            return json_encode($alerta);
            exit();
